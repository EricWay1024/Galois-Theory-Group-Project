...
\section{Polynomials}
In this section we first give some basic definitions involving polynomials. Then we look at theorems that deal with the irreducibility of polynomials over $\Z$ and $\Q$ before briefly recalling the Fundamental Theorem of Algebra for polynomials over $\C$. This section is based on \cite{Stewart}. 
...
This is a special case where it only works over $\C$. Such a number $z_0$ is called a root of  $f(t)$. For example, $i$ is a root of the polynomial $t^2+1$. Polynomials may have more than one root; indeed, $t^2+1$ has at least one other root, $-i$ but others will have more roots. We do the following proof by starting out with a contradiction.
...
\begin{proof}
	Use induction on $n$. When $n = 1$ then it is obvious that this proposition follows. If $n > 1$ then we know by Theorem \ref{thm:fundamental-algebra}, that $f(t)$ has at least one root $\alpha_n$. By Theorem \ref{thm:remainder}, there exists $q(t) \in \C[t]$ such that,
		$f(t) = (t-\alpha_n) q(t)$. 
	Then $\partial q = n - 1$, so by induction,
		$q(t) = k(t-\alpha_a)...(t-\alpha_{n-1})$. 
	For suitable complex numbers $k,\alpha_1,...,\alpha_{n-1}$. Substitute $q(t)$ into $f(t)$ and the induction step is complete.
\end{proof}