...
Given a field $L$, we now look at subgroups of $\Aut(L)$ and the elements in $L$ such a subgroup fixes. We investigate the matter with two approaches. On the one hand, we might choose a subfield $K$ of $L$ and define $G$ to be the group of all $K$-automorphisms of $L$, so that $G$ is guaranteed to fix $K$ pointwise, and we say $G$ is the \textit{Galois group} of the field extension $L/K$, denoted as $G = \Gal(L/K)$. We also define the Galois group of a polynomial $f$ over $K$ with splitting field $F$ as the Galois group of $F / K$, and this group acts on the roots of $f$ naturally. On the other, we might choose any $G \le \Aut(L)$, and define $K$ to be the subfield of $L$ fixed by all elements in $G$, denoted as $K = \Fix(G)$, in which case $K$  is the \textit{fixed field} of $G$. In general, $\Fix(\Gal(L/K)) \neq K$. In fact, the condition for the Galois correspondence $\Fix(\Gal(L/K)) = K$ is when $L/K$ is a finite `Galois' extension (defined to be both normal and separable). 
...
\begin{proof}
	Each element $a \in L$ can be represented as $ f(\alpha_1, \dots, \alpha_n) / g(\alpha_1, \dots, \alpha_n)$, where $f, g \in K[t_1, \ldots, t_n]$. Since $\sigma$ and $\tau$ preserve arithmetic operations, fix all elements in $K$, and act in the same way on $\alpha_i$, it is clear that
	\begin{equation*}
		\begin{split}
		\sigma(a) 
...
We now show that a $K$-monomorphism always maps a root of a polynomial over $K$ to another, a general setting applicable to the Galois group of a field extension $L/K$ \citep[p.~16]{morandi_field_1996}. 
...
The Galois group of a polynomial naturally leads to a group action on its roots. The next theorem is inspired by Macauley \cite{visual-algebra}. 
...
\begin{example} \label{exm:galois-group}
	...
	Let us determine $\Gal(f)$. Let $\sigma \in \Gal(f)$, then $\sigma$ is completely determined by $\sigma(\sqrt 2)$ and $\sigma(i)$. Since $\sigma(\sqrt 2)$ has the same minimal polynomial over $\Q$ as $\sqrt 2$, which is $t^2 - 2$, $\sigma(\sqrt 2) = \pm \sqrt 2$. Similarly, $\sigma(i) = \pm i$. Define $\phi, \psi \in \Gal(f)$ such that 
	$$
...
We now prove an upper bound for the order of the Galois group of a finite field extension, formalising ideas from  \cite[Lecture~8]{galois-theory-lectures}. As we see in the next subsection, this upper bound is reached exactly when $L/K$ is a Galois extension. 

\begin{lemma} \label{thm:galois-group-order-upper-bound}
	Let $L/K$ be a finite field extension and let $G = \Gal(L/K)$, then $|G| \le [L:K]$. 
\end{lemma}

\begin{proof}
	Since $L/K$ is finite, by Theorem \ref{thm:finite-equi-def}, we can write $L = K(\alpha_1, \ldots, \alpha_r)$ for $r$ finite and $\alpha_i$ algebraic over $K$. Denote $K_0 = K$ and
	$K_i = K(\alpha_1, \dots, \alpha_i)$ for $i = 1, \ldots, r$. In particular, $K_r = L$. 
	
	
	For any field $F$ such that $K \subseteq F$, consider a $K$-monomorphism $\phi_0 : K \to F$. For $i = 1, \ldots, r$, consider the extension $K_i / K_{i-1}$, where $K_i = K_{i-1} (\alpha_i)$. Let $\alpha_i$ be a root of an irreducible polynomial $f_i$ over $K$ of degree $\partial f_i = [K_i : K_{i-1}]$ by Theorem \ref{thm:degree-theorem}. $f_i$ is over $K$ and hence is fixed under any $K$-monomorphism. We now extend $K$-monomorphism $\phi_{i-1} : K_{i-1} \to F$ to $K$-monomorphism $\phi_i: K_i \to F$, then $\phi_i$ maps $\alpha_i$ to a root of $f_i$ in $F$ by Theorem \ref{thm:galois-group-permutes-zeros}, and the number of roots of $f_i$ in $F$ is \textit{at most} $\partial f_i$. Hence there are at most $[K_i : K_{i-1}]$ ways to extend $\phi_{i-1}$ to $\phi_i$. There are at most $\prod_{i=1} ^r [K_i : K_{i-1}] = [K_r : K_0] = [L : K]$ ways to extend $\phi_0$ to $\phi_r$ by Theorem \ref{thm:tower-theorem}. Since $\phi_r : L \to F$, if we let $F = L$, then $\phi_r \in \Gal(L/K)$. Also, there is only one way to construct $\phi_0$. Thus each way of extending $\phi_0$ to $\phi_r$ corresponds to an element in $\Gal(L/K)$, and thus $\Gal(L/K) \le [L:K]$. 
\end{proof}
...
In particular, for a finite field extension $L/ K$, the Galois group $G = \Gal(L / K)$ is a subgroup of $\Aut(L)$ such that $K \subseteq \Fix(G) \subseteq L. $ We are mainly interested in cases where $K = \Fix(G) = \Fix(\Gal(L/K))$. This is true exactly when $L/K$ is a Galois extension, as we see in the next subsection. To show this is not true in general, we now look at a counterexample. 

\begin{example}
   Consider when $K = \Q$ and $L = \Q (\alpha)$ where $\alpha = \sqrt[3]{2}$. If $\sigma \in G$, then $\sigma(\alpha)$ is a root of the polynomial $t^3 - 2$ by Theorem \ref{thm:galois-group-permutes-zeros}, but the two non-real roots are not contained in $L$. Thus $\sigma(\alpha) = \alpha$ and $G = \{ \Id \}$. Thus $\Fix(G) = L \neq K$. 
\end{example}
