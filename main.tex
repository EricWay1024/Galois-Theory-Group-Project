...
%	If $L/K$ is a Galois extension and $M$ is an intermediate field such that $K \subseteq M \subseteq L$, then $L/M$ is a Galois extension. 
\end{theorem}
...
\begin{theorem}[The Fundamental Theorem of Galois Theory] \label{thm:fixed}
...
Now we are ready to work on the second part using what we have already obtained:

\begin{proof}[Proof, second part]
	We first show (i) $\Rightarrow$ (v):
	
%	Denote \begin{itemize}
%		\item $\alpha:M \mapsto \Gal(L/M) \subseteq G$ and
...
%	we now show 
	...
	\begin{itemize}
		...
		\item ($\beta \circ \alpha = \Id$) Let $M$ be an intermediate field, and thus $L/M$ is a Galois Extension. Then if we consider the Galois group $\Gal(L/M)$, we can see that via Theorem \ref{thm:fixed}, $M = \Fix(\Gal(L/M))$. Since $\alpha(M) = \Gal(L/M)$, we look at $(\beta \circ \alpha)(M)$, which is equal to $\beta(\Gal(L/M)) = \Fix(\Gal(L/M))$. Thus $(\beta \circ \alpha)(M) = M$ and hence we see that $\beta \circ \alpha = \Id$.
		
		\item ($\alpha \circ \beta = \Id$) Now, let $H$ be a subgroup of $G$ and then set $M :=\Fix(H)$ in order that $L/M$ is a Galois extension. This implies that $H$ is a subgroup of the Galois group $\Gal(L/M)$. From Theorem \ref{thm:fixed}, we have that $\Fix(\Gal(L/M))=M$, and we also have that $\Fix(H) = M$ so we can say that $\Fix(H)=\Fix(\Gal(L/M)) \implies H = \Gal(L/M) = \Gal(L/\Fix(H))$. Thus if we consider $(\alpha \circ \beta)(H) = \alpha(\Fix(H)) = \Gal(L/\Fix(H))$, and as we have just seen above $H = \Gal(L/\Fix(H))$, so we have $(\alpha \circ \beta)(H) = H$ and so $\alpha \circ \beta = \Id$.
	\end{itemize}
