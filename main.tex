...
\begin{definition}
    Let $K$ be a subfield of $\C$. A polynomial $f$ over $K$ \textit{splits} in $K$ if it can be expressed as $$
    f(t) = k \prod _{i=1} ^n (t - \alpha_i),
    $$
    where $k, \alpha_1, \ldots, \alpha_n \in K$. 
\end{definition}
Theorem \ref{thm:fundamental-algebra} indicates that for $f$ splits over $K$ if and only if all of its zeros are contained in $K$. 
...
\begin{definition}
    ...
    \end{definition}
    % didn't define "smallest" field extension
    \begin{example}
...
\end{example}
\begin{theorem}
    A  field extension $L/K$ is normal and finite if and only if $L$ is the splitting field for some polynomial $f$ over $K$.
\end{theorem}
...
\begin{definition}
    Let $f$ be an irreducible polynomial over field $K$ with a splitting field contained in $L$. Then $f$ is separable over $K$ if $f$ takes the form 
    $$
        f(t) = k(t - \sigma_1) \dots (t - \sigma_n)
    $$
    where $\sigma_i \in L$ are distinct.
\end{definition}
...
\begin{definition} \label{def:separable-extension}
    A field extension $L / K$ is \textbf{separable} if for any $\alpha \in L$, the minimal polynomial of $\alpha$ over $K$ is separable over $K$. 
\end{definition}
...
For a finite field extension $L/ K$, the Galois group $G = \Gal(L / K)$ is a subgroup of $\Aut(L)$ such that $$K \subseteq \Fix(G) \subseteq L. $$ We are mainly interested in the case where $K = \Fix(G)$.  
...
To ensure that $K = \Fix(G)$, certain requirements must be met by the finite field extension $L / K$. Specifically, it must be both normal and separable. 

\begin{definition}
A field extension is said to be a \textbf{Galois extension} if it is finite, normal and separable. 
% For $L/K$ Galois, we have that $\Aut(L/K)$ is the \textbf{Galois group} and denoted by $\Gal(L/K).$
\end{definition}
...
First, we need to understand two theorems in order to be able to prove the Abel-Ruffini Theorem. The first is Hilbert's Theorem \textcolor{red}{May be Artin} which shows that general polynomials have a Galois Group that is isomorphic to $S_n$, the proof of which has been adapted from Lorenz \cite{Lorenz}.

\begin{theorem}[Hilbets/Artin]
   Theorem Goes Here
\end{theorem}
...
\begin{proof}\label{thm:abel-ruffini-thm}
    By Hilbert's Theorem, we know that the general polynomial, written in the form $f(x)=a_0X^n - a_1X^{n-1} + ... + (-1)^na_n$ has Galois group $S_n$. Then by Theorem \ref{thm:symmetric-not-soluble} we can see that for $n \geq 5$, $S_n$ is not soluble, which by Theorem \ref{thm:galois-theorem} implies that the general polynomial of degree $n \geq 5$ is not solvable.
\end{proof}
