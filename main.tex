...
To ensure that $K = \Fix(G)$, certain requirements must be met by the finite field extension $L / K$. Specifically, it must be both normal and separable, as defined below.

\begin{definition}
    A finite field extension $L/K$ is \textbf{normal} if $L$ is the splitting field for some polynomial $f$ over $K$.
\end{definition}
